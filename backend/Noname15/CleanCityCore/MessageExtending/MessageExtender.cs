...
namespace CleanCityCore.MessageExtending
{
    public class MessageExtender : IMessageExtender
    {
        ...
        public string ExtendReportText(Report report)
        {
            ...
            {
                returnMsg +=
                    "На основании пункта 7 РЕШЕНИЯ от 26 июня 2012 года N 29/61 \"Об утверждении правил благоустройства территории муниципального образования город Екатеринбург\" на территории муниципального образования город Екатеринбург запрещается:[DELIM]";
                var id = 1;
                foreach (var item in returnDeny)
                {
                    returnMsg += $"{id}. {item}[DELIM]";
                    id += 1;
                }
            }
